			...
			if ( $get_filters ) {
				...
				foreach ($filters as $filter_slug) {
					...
					if ( !is_wp_error( $filter_object ) && is_object($filter_object) ) {
						$filters_item_out .= '<li class="item-'. $filter_i . _if( $filter_count == $filter_i, ' last-item' ).'"><a href="javascript:;" data-filter=".filter-'. $filter_object->term_id .'">'. $filter_object->name .'</a></li>';
					}
				}
